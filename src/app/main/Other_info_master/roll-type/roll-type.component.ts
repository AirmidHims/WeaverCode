...
import { EditRolltypeComponent } from './edit-rolltype/edit-rolltype.component';
import { NewRolltypeComponent } from './new-rolltype/new-rolltype.component';

@Component({
  selector: 'app-roll-type',
  templateUrl: './roll-type.component.html',
  styleUrls: ['./roll-type.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class RollTypeComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<RollTypemaster>();
  ...
  ngOnInit(): void {
    ...
    console.log(D_data);
    this.D_data1 = D_data;
    this._OtherinfoMasterService.getRollTypeList(D_data).subscribe(Visit => {
      this.dataSource.data = Visit as RollTypemaster[];
  ...
  NewRollTypemaster() {
    const dialogRef = this._matDialog.open(NewRolltypeComponent,
      {
        maxWidth: "45vw",
        height: '450px',
        width: '100%',
  ...
  onEdit(row){
    console.log(row);
      ...
      const dialogRef = this._matDialog.open(EditRolltypeComponent, 
        {   maxWidth: "45vw",
            height: '450px',
            width: '100%',
             data : {
...
//  let Query ="update RollTypemaster set isActive=0 where yID=" +element.yID + "";
// console.log(Query);
//  this._OtherinfoMasterService.getDeleteRollTypemaster(Query).subscribe(data => {
//   if(data)
...
export class RollTypemaster {
   ...
   * @param contact
   */
  constructor(RollTypemaster) {
    {
      this.RollTypeId = RollTypemaster.RollTypeId || 0;
      this.RollTypeCode = RollTypemaster.RollTypeCode || '';
      this.RollType = RollTypemaster.RollType || 0;
