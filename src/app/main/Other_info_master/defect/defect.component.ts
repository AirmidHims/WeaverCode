...
export class DefectComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<Defectmaster>();
  ...
  ngOnInit(): void {
    ...
    console.log(D_data);
    this.D_data1 = D_data;
    this._OtherinfoMasterService.getDefectList(D_data).subscribe(Visit => {
      this.dataSource.data = Visit as Defectmaster[];
  ...
  NewDefectmaster() {
    const dialogRef = this._matDialog.open(NewDefectComponent,
      {
        maxWidth: "45vw",
        height: '450px',
        width: '100%',
  ...
  onEdit(row){
    // console.log(row);
      ...
      const dialogRef = this._matDialog.open(EditDefectComponent, 
        {   maxWidth: "45vw",
            height: '450px',
            width: '100%',
             data : {
...
//  let Query ="update Defectmaster set isActive=0 where yID=" +element.yID + "";
// console.log(Query);
//  this._OtherinfoMasterService.getDeleteDefectmaster(Query).subscribe(data => {
//   if(data)
...
export class Defectmaster {
   ...
   * @param contact
   */
  constructor(Defectmaster) {
    {
      this.DefectId = Defectmaster.DefectId || 0;
      this.DefectCode = Defectmaster.DefectCode || '';
      this.DefectName = Defectmaster.DefectName || 0;
      this.EntNo = Defectmaster.EntNo || 0;
