...
import { NewLoomTypeComponent } from './new-loom-type/new-loom-type.component';

@Component({
  selector: 'app-loomtype',
  templateUrl: './loomtype.component.html',
  styleUrls: ['./loomtype.component.scss'],
...
export class LoomtypeComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<LoomType>();
  menuActions: Array<string> = [];
  ...
  NewLoomType() {
    const dialogRef = this._matDialog.open(NewLoomTypeComponent,
      {
        maxWidth: "45vw",
        height: '450px',
        width: '100%',
  ...
  onEdit(row){
    console.log(row);
      var m_data = {
        ...
        "LoomTypeCode":row.LoomTypeCode,
        "LoomType":row.LoomType,
               
      }
    
      console.log(m_data);
      this._OtherinfoMasterService.populateFormAll(m_data);
      
      const dialogRef = this._matDialog.open(EditLoomTypeComponent, 
        {   maxWidth: "45vw",
            height: '450px',
            width: '100%',
             data : {
...
//  let Query ="update LoomType set isActive=0 where yID=" +element.yID + "";
// console.log(Query);
//  this._OtherinfoMasterService.getDeleteLoomType(Query).subscribe(data => {
//   if(data)
...
export class LoomType {
   ...
   * @param contact
   */
  constructor(LoomType) {
    {
      this.LoomTypeId = LoomType.LoomTypeId || 0;
      this.LoomTypeCode = LoomType.LoomTypeCode || '';
      this.LoomType = LoomType.LoomType || 0;
      this.EmptyBeamWt = LoomType.EmptyBeamWt || 0;
